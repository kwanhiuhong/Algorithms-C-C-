...
//  AVL_Tree
//
//  Created by KHH on 29/11/2018.
//  Copyright © 2018 Kwan Hiu Hong. All rights reserved.
//
#include <iostream>
#include <stdio.h>
#include <assert.h>
...
void in_order_search(node *&ref_root){
    if(ref_root == NULL){
        return ;
    }
    in_order_search(ref_root->left);
    cout << ref_root->key << ' ';
//    cout << ref_root->size << endl;
    in_order_search(ref_root->right);
}
...
void right_rotate(node*& ref_root)
{
    ...
    //cout << "Right_rotation on root: " << ref_root->key << endl;
...
void left_rotate(node*& ref_root)
{
    ...
    //cout << "Left_rotation on root: " << ref_root->key << endl;
    ...
    /*done write something here. Checked-completed*/
}
...
bool insert_key(int key, node*& ref_root)
{
    ...
    if(key < ref_root->key){
        ...
        if (insert_key(key, ref_root->right)) {
            /* write something here. done-Checked-completed*/
            ...
            if (ref_root->height == RightHigher) {
                ...
                // If tree changes to LeftHigher from previously Balanced state, this means that tree's height is increased by 1.
                return ref_root->height == RightHigher;
            }
...
int KthMin(node * root, int k){
    ...
    node *temp = root;
    if(k == temp->size - sizeOfTree(root->right)){
        return temp->key;
    }
    while(1){
        if(k <= 0){
        ...
        if(k <= lsize){
            ...
            }else if(k == temp->size - sizeOfTree(temp->right)){
                return temp->key;
            }
            temp = temp->left;
            lsize = sizeOfTree(temp->left);
            continue;
        }else if(k > lsize){
            //output = temp;
            if(k == temp->size - sizeOfTree(temp->right)){
                return temp->key;
            }
            temp = temp->right;
            k = k - lsize - 1;
...
int main()
{
    ...
    while(true)
    {
        ...
        switch(op[0]) {
            ...
            case 'Z': in_order_search(root); cout << "\n"; break;
            default: assert(0);
        }
    }
    return 0;
